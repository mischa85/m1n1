...
#define T6041_PLANE_OFFSET  0
#define T6041_PLANE_STRIDE  0x40000
#define T6041_GLOBAL_OFFSET 0x100000
#define T6041_DCS_OFFSET    0x400000
#define T6041_DCS_STRIDE    0x200000
...
// Apparently this translates to this. But nobody wrote down *how* to convert these...
// "region-id-2" = <00404eda1f0100000000802500000000>
// "region-id-4" = <00805aca1f0100000080f00100000000>
...
#define T6041_CACHE_STATUS_DATA_COUNT GENMASK(13, 9)
#define T6041_CACHE_STATUS_TAG_COUNT  GENMASK(8, 4)
...
int mcc_init_t6041(int node, int *path)
{
    ...
    printf("MCC: Initializing T6041 MCCs (%d instances)...\n", mcc_count);
    ...
    for (int i = 0; i < mcc_count; i++) {
        ...
        mcc_regs[i].plane_base = base + T6041_PLANE_OFFSET;
        mcc_regs[i].plane_stride = T6041_PLANE_STRIDE;
        mcc_regs[i].plane_count = plane_count;

        mcc_regs[i].global_base = base + T6041_GLOBAL_OFFSET;

        mcc_regs[i].dcs_base = base + T6041_DCS_OFFSET;
        mcc_regs[i].dcs_stride = T6041_DCS_STRIDE;
        mcc_regs[i].dcs_count = dcs_count;
